...
export async function scrapeGoodreadsByIsbn(payload) {
  const searchUrl = `https://www.goodreads.com/search?q=${payload.isbn}`;
  return await goodreadsUrlToBook(searchUrl);
}

export async function scrapeGoodreadsByTitleAuthor(payload) {
...
export async function goodreadsUrlToBook(bookUrl: string) {
